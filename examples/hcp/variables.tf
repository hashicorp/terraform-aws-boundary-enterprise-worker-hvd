...
variable "kms_worker_arn" {
  type        = string
  description = "KMS ARN of the worker-auth kms key."
  default     = ""
  validation {
    condition     = var.worker_registration_method == "kms-led" ? can(regex("^arn:aws:kms:[a-z0-9-]+:[0-9]+:key/[a-f0-9-]+$", var.kms_worker_arn)) : true
    error_message = "KMS worker ARN must be a valid KMS key ARN when `worker_registration_method` is set to `kms-led`."
  }
}
...
variable "kms_endpoint" {
  ...
  default     = ""
  validation {
    condition     = var.worker_registration_method == "kms-led" ? can(regex("^[a-z0-9.-]+\\.[a-z0-9.-]+\\.amazonaws\\.com(\\.cn)?(:[0-9]+)?$", var.kms_endpoint)) : true
    error_message = "KMS endpoint must be a valid AWS service endpoint when `worker_registration_method` is set to `kms-led`."
  }
}

variable "controller_generated_activation_token" {
  type        = string
  description = "Activation token generated by the controller. Required if `worker_registration_method` is set to `controller-led`."
  default     = ""
  validation {
    condition     = var.worker_registration_method == "controller-led" ? length(var.controller_generated_activation_token) > 0 : true
    error_message = "Controller-generated activation token must be provided when `worker_registration_method` is set to `controller-led`."
  }
}
