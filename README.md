...
1. Populate your own custom values into the `terraform.tfvars.example` file that was provided, and remove the `.example` file extension such that the file is now named `terraform.tfvars`. Ensure to set the `hcp_boundary_cluster_id` variable with the Boundary Cluster ID from step 1.
    ...
    >📝 Note: The `friendly_name_prefix` variable should be unique for every agent deployment.
     ...
     If using controller-led worker authorization, follow [these instructions](https://developer.hashicorp.com/boundary/docs/workers/registration#controller-led-authorization) to use the Boundary CLI (UI not currently supported) to get the Boundary controller PKI token from the HCP server.  Make sure to set the `controller_generated_activation_token` variable to the value returned by the CLI.

     If using worker-led worker authorization [details here](https://developer.hashicorp.com/boundary/docs/workers/registration#worker-led-authorization), leave the `controller_generated_activation_token` variable empty. You'll complete the registration in step 11 below.

    >📝 Note: kms-led worker authorization is not currently supported for HCP Boundary
...
| <a name="input_controller_generated_activation_token"></a> [controller\_generated\_activation\_token](#input\_controller\_generated\_activation\_token) | Activation token generated by the controller. Required if `worker_registration_method` is set to `controller-led`. | `string` | `""` | no |
...
| <a name="input_kms_worker_arn"></a> [kms\_worker\_arn](#input\_kms\_worker\_arn) | KMS ARN of the worker-auth kms key. | `string` | `""` | no |
...
| <a name="input_worker_registration_method"></a> [worker\_registration\_method](#input\_worker\_registration\_method) | Method for registering the worker with the Boundary cluster. Supported values are `worker-led`, `controller-led`, and `kms-led`. | `string` | `"worker-led"` | no |
